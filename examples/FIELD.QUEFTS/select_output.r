select_output <- function(Store, fields, seasons) {
...
for (field in seq (1,fields,1)) {
  ...
  Output[,2,field]  <- Store[[field]]$crop$resourceLimitedYield
  Output[,5,field]  <- Store[[field]]$soil$C$totalSoilC
  Output[,6,field]  <- Store[[field]]$crop$waterLimitedYield
}
